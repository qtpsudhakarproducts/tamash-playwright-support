...
- **`expect(locator).toBeVisible()` and similar assertions** — same reasoning, and they never reach this mechanism anyway (Playwright's own matcher machinery). A genuinely-failed assertion still gets useful information, just not a fix: see [Understanding why a test failed](reports.html#understanding-why-a-test-failed) — a separate, TypeScript-only capability that classifies *why* it failed instead of silently retrying it.
